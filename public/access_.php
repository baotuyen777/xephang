...
function access($stt,$tg)
{
    ...
    $phieuKhamQuery = oci_parse($conn, "SELECT PHIEU_KHAM_ID,BENH_NHAN_ID,BPHAN_KHAM_ID FROM PHIEU_KHAM WHERE SO_PHIEU_TG = '$tg' AND SO_PHIEU_STT = '$stt'");
    oci_execute($phieuKhamQuery);
    $row = oci_fetch_row($phieuKhamQuery);
    if($row != false){
        $PHIEU_KHAM_ID = $row[0];
        $arrayOutInfo = getInfo($conn,$row[1],$row[2]);
    }
...
function getInfo($conn,$BENH_NHAN_ID,$BPHAN_KHAM_ID){
    $outInfo = array();
    $infoQuery = oci_parse($conn, "SELECT TEN_BNHAN,NGAY_SINH,GIOI_TINH,DUONG_PHO FROM BENH_NHAN WHERE BENH_NHAN_ID = '$BENH_NHAN_ID'");
