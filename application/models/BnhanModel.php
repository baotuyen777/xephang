...
{
	public function getBNhanByRoom($room,$type = 0){
        $date = date('d-m-Y',time());
        $startTime = strtotime($date);
        $endTime = $startTime + 86000; 
		$db = $this->getSlaveDb();
		$sql = "SELECT bq.*,b.NGAY_SINH,b.GIOI_TINH FROM BENH_NHAN_QUEUE as bq INNER JOIN BENH_NHAN as b ON bq.BENH_NHAN_ID = b.BENH_NHAN_ID WHERE bq.room = ? AND bq.type = ? AND bq.created >= $startTime AND bq.created < $endTime GROUP BY b.tc ORDER BY bq.sort ASC";
		//echo $sql;exit;
        return $db->fetchAll($sql,array($room,$type));
	}
    ...
    public function getBNhanQueu($BENH_NHAN_ID,$room){
        $db = $this->getSlaveDb();
        $sql = "SELECT * FROM BENH_NHAN_QUEUE WHERE BENH_NHAN_ID = ? AND ROOM = ?";
        return $db->fetchRow($sql,array($BENH_NHAN_ID,$room));
    }
}